...
resource "aws_vpc" "fleet" {
  ...
  tags = merge(var.tags, { Name = "nginx-pqc-perf-test-fleet" })
}
...
resource "aws_security_group" "client" {
  name_prefix = "nginx-pqc-fleet-client-"
  description = "nginx-pqc-perf-test fleet bench client"
  vpc_id      = aws_vpc.fleet.id
  ...
  tags = merge(var.tags, { Name = "nginx-pqc-perf-test-fleet-client" })
}

# --- nginx targets ----------------------------------------------------------
resource "aws_security_group" "target" {
  name_prefix = "nginx-pqc-fleet-target-"
  description = "nginx-pqc-perf-test fleet nginx target"
  vpc_id      = aws_vpc.fleet.id
  ...
  tags = merge(var.tags, { Name = "nginx-pqc-perf-test-fleet-target" })
}
