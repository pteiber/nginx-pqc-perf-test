# Fleet mode root module. Wires a shared network, N nginx target hosts
# (one per nginx_targets entry), and a single bench client that drives
# every benchmark over the network and consolidates the results. After
...
resource "aws_key_pair" "generated" {
  key_name   = "nginx-pqc-perf-test-fleet-${substr(md5(tls_private_key.ssh.public_key_openssh), 0, 8)}"
  public_key = tls_private_key.ssh.public_key_openssh

  tags = merge(local.common_tags, { Name = "nginx-pqc-perf-test-fleet" })
}

resource "local_sensitive_file" "private_key" {
  filename        = "${path.module}/generated/nginx-pqc-perf-test-fleet.pem"
