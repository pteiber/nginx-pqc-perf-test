...
variable "run_ansible" {
  description = "If true (the default), Terraform runs the Ansible playbook automatically after the instances are provisioned, using the generated inventory. Set to false to provision infrastructure only and run Ansible yourself against aws/fleet/ansible/inventory.ini."
  type        = bool
  default     = true
}
