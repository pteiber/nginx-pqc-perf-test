...
#
# Rocky Linux 9 is Marketplace-only; the AWS account must accept the
# "Rocky Linux 9" subscription once per account, or apply fails with
# OptInRequired. See aws/README.md. Rocky publishes the two architectures
# under different accounts: arm64 (aarch64) under 679593333241, x86_64
# under 792107900819. Both are listed as owners; the architecture filter
# below selects the right one (only one account has images per arch).
locals {
