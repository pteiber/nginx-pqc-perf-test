...
Three self-contained AWS modes live under `aws/`; they share no Terraform or Ansible with each other, only the repo-root building blocks above (`bench/`, `trafficmix/`, `certs/gen-certs.sh`, `html/small.html`, `scripts/summarize.sh`). Keep them separate when editing: do not factor shared infra code across them.
...
- **Traffic-mix** (`aws/traffic-mix/`): the same client-plus-N-targets topology as Fleet (copied from it, then adapted; the two share nothing), but the client runs the separate `trafficmix/` load generator instead of `bench/`, driving a realistic mixed workload with four independent knobs: HTTP request rate over a warm keep-alive pool, new-connection TLS handshake rate, the percentage of those handshakes that resume a TLS 1.3 session, and a random response-size range served via HTTP `Range` requests against a single `payload.bin`. Two load-bearing target-config differences from Fleet, both required by the workload: the nginx configs **enable** session resumption (`ssl_session_cache` + `ssl_session_tickets on`, which Fleet turns off) and serve a large `payload.bin` (generated by Ansible, size `payload_bytes`, default 4 MiB). It reuses `target-stats.sh` verbatim and has its own `run-traffic-mix-benchmark.sh` + `summarize-traffic-mix.sh`. The PQC-vs-Classic invariant still holds within the mode: the two target configs differ only in the `ssl_conf_command Groups` line. The workload knobs are runtime env vars on the client (`BENCH_RPS`, `BENCH_HANDSHAKE_RATE`, `BENCH_RESUME_PCT`, `BENCH_MIN_BYTES`, `BENCH_MAX_BYTES`, `BENCH_CONNS`, `BENCH_HANDSHAKE_CONNS`, `BENCH_DURATION`), not Terraform vars. Note `trafficmix/` is its own Go module (`go 1.24`, same reason as `bench/`) and cannot import `bench/`; it copies the shared latency/percentile/JSON-writing patterns.
...
Same shape as Fleet (Ansible auto-runs on `terraform apply`; the run is by hand on the client), just a different working dir, run script, and knobs. The targets here have session resumption enabled and serve a `payload.bin`.

```sh
cd aws/traffic-mix/terraform
cp terraform.tfvars.example terraform.tfvars
# edit terraform.tfvars: set allowed_ssh_cidr, owner, email, and the nginx_targets map
# (same nginx_targets format as fleet mode)
terraform init
terraform apply -auto-approve   # provisions all hosts AND runs Ansible via a local-exec

# run the mixed-workload benchmark FROM the client (it loads every target over the network):
SSH_CMD="$(terraform output -raw client_ssh_command)"
...
The four workload knobs plus pool sizes and duration are runtime env vars on the client (no sudo, so a plain prefix works): `BENCH_RPS` (default 200), `BENCH_HANDSHAKE_RATE` (50), `BENCH_RESUME_PCT` (50), `BENCH_MIN_BYTES` (1024), `BENCH_MAX_BYTES` (65536, must be <= the served `payload_bytes`, default 4 MiB), `BENCH_CONNS` (50, keep-alive pool), `BENCH_HANDSHAKE_CONNS` (50), `BENCH_DURATION` (30s). Example: `BENCH_RPS=1000 BENCH_HANDSHAKE_RATE=200 BENCH_RESUME_PCT=80 BENCH_DURATION=60s /opt/nginx-pqc-perf-test/run-traffic-mix-benchmark.sh`. To serve responses larger than 4 MiB, re-provision with a larger payload via `-e payload_bytes=<bytes>` on the Ansible run. `summary.md` reports, per host+scenario, the *achieved* request and handshake rates, request latency percentiles, actual resume percentage, full-vs-resumed handshake latency, errors, and a Dropped count (nonzero = the requested rate could not be met, i.e. client or target saturated). Same Ansible-retry note as Fleet mode.
